...
2. **PUA检测** — 扫描文档中的私用区字符（U+E000-U+F8FF），通过系统字体渲染和语句匹配映射为标准 Unicode
3. **清洗切分** — 去除多余空白、统一标点、按篇章切分、区分正文与译文
4. **模板排版** — 复制已有成品 docx 作为模板，替换正文内容（保留页面边框、印章、页码等）
5. **段落优化** — 合并碎片段落（对话标记分段、问答对合并）
6. **字库检查** — 比较文档用字与字库覆盖范围，列出缺失字符
7. **繁简转换** — 语义感知的简繁转换，正确处理一对多映射
8. **竖排标点** — 标点缩小淡化、字形替换（引号→角括号、书名号→浪线、括号→直式）
9. **智能目录** — 生成可更新的 TC + TOC 域目录
